    ...
    public class ItemService : IItemService
    {
        ...
        public IEnumerable<Item> GetAllByLocation(string location)
        {
            if (string.IsNullOrWhiteSpace(location))
                return Db.Items.Include(x => x.Restaurant).Where(x => x.Restaurant != null).OrderByDescending(x=>x.IsFeaturd).AsNoTracking().ToList();
            return Db.Items.Include(x => x.Restaurant).Where(x => x.Restaurant != null && x.Restaurant.Location.Contains(location)).OrderByDescending(x => x.IsFeaturd).AsNoTracking().ToList();
        }
